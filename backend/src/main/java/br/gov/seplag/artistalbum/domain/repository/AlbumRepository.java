...
public interface AlbumRepository extends JpaRepository<Album, Long> {
    ...
    @Query("SELECT a FROM Album a WHERE a.artist.id = :artistId AND LOWER(a.title) LIKE LOWER(CONCAT('%', :title, '%'))")
    ...
    @Query("SELECT COUNT(a) > 0 FROM Album a WHERE LOWER(a.title) = LOWER(:title) AND a.artist.id = :artistId AND a.id != :id")
    boolean existsByTitleAndArtistIdAndIdNot(@Param("title") String title, @Param("artistId") Long artistId, @Param("id") Long id);

    @Query("SELECT COUNT(a) > 0 FROM Album a WHERE LOWER(a.title) = LOWER(:title) AND a.artist.id = :artistId")
